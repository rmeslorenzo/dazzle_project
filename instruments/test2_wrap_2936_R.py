...
from dazzle_project.instruments.base_powermeter import BasePowerMeter, CommError, PowerMeterLibraryError
...
class PowerMeter(BasePowerMeter):
    ...
    def __init__(self, pid=0xcec7, *setupArgs):
        ...
        r""" initializes the power meter object. pid is the product id for the power meter from the file
    ...
    def _setupMeter(self, wavelength=1300, autoRange=0, range=1, filterType=1, analogFilter=4, digitalFilter=10000,
                    units=2):
    ...
    def readPowerWithStatus(self):
        ...
        statusDict = {"units": status[0], "range": status[1], "detectorPresent": status[2], "ranging": status[3],
                      "saturated": status[4], "overange": status[5]}
        ...
        if n == 1:
            ...
            if timeout:
                raise CommError(
                    "Expected "
                    + str(n)
                    + " samples from power meter, but only received "
                    + str(numValues)
                )
    ...
    def readPower(self, tau=200, mode="mean"):
        ...
        elif self.range > 0 and maxPower < 0.99 * self.rangeDic[self.range - 1] and self.t0 and (
                time.time() - self.t0) < self.timeout:
            self.setRange(self.range - 1)
            time.sleep(1)
            return self.readPower(tau, mode)
...
class USBConnection(object):
    ...
    def __init__(self, pid):
        ...
        if s < 0:
            raise CommError("Connection to pid=" + str(
                pid) + " successful, but get_device_info failed and returned " + str(s))
