...
class Newport_2936R():
    ...
    def close_device(self):
        ...
        if status != 0:
            print(f"device closed but it should return 0 not {status}")
            raise CommandError(f"device closed but it should return 0 not {status}")
        else:
...
if __name__ == '__main__':
    ...
    if nd.status == 'Connected':
        ...
        print(f'Connected to {str(nd.ask('*IDN?'))}')
